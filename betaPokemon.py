...
if __name__ == '__main__':
   ...
   Pokemonlist = [
    ...
    Pokemon('Venasaur', 'Grass',
       ['Solar Beam', 'Energy Ball', 'Freny Plant', 'Bullet Seed'], 9, 14, 100)
   ]
#TO DO: Give pokemon moves with separate types
   userChoice = str(
     input(
          'Choose a Pokemon from this very short list. Charizard, Blastoise, or Venasaur: '
      ))
   while userChoice not in "CharizardcharizardBlastoiseblastoiseVenasaurvenasaur":
    userChoice = str(
    input(
        'Choose a Pokemon from this very short list. Charizard, Blastoise, or Venasaur: '
      ))
   ...
   elif userChoice == 'Blastoise':
    userChoice = Pokemonlist[1]
   elif userChoice == 'Venasaur':
